...
let mockedModules = jest.createMockFromModule('@mui/x-date-pickers/MobileDateTimePicker');

mockedModules.MobileDateTimePicker = ({ children, onClose, onChange, ...props }) => {
	let { newProps, childProps } = transformProps(props);

	return (
		<div {...newProps} data-testid="MobileDateTimePicker" onClick={onClose}>
