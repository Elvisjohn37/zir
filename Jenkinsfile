pipeline {
  environment {
    DOCKER_IMAGE = "nexus01.leekie.com:8483/zircon_ps_cw"
    DEPLOYMENT = "zircon_ps"
  }
  agent any
  stages {
    stage('Get Version') {
    ...
    stage('Deploy To UAT') {
      ...
      steps {
        ...
        withDockerRegistry([credentialsId: 'qauser', url: "https://nexus01.leekie.com:8483"]) {
        sh """
          if ! docker manifest inspect ${DOCKER_IMAGE}:${COMMIT} --insecure > /dev/null; then
            svn upgrade
    ...
    stage('Deploy To Production') {
      ...
      steps {
        script {
          emailext to: 'onyx.ps@leekie.com',
  ...
  post {
    failure {
      emailext body: "Please check on revision ${env.COMMIT}", subject: "Build failed on $DEPLOYMENT", to: "onyx.ps@leekie.com"
    }
  }
}